...
class Settings:
    ...
    chunk_overlap: int = int(os.environ.get("CHUNK_OVERLAP", 350))
    ...
    top_k: int = int(os.environ.get("TOP_K", 12))  # Increased slightly for better coverage while staying within context window
    score_threshold: float = float(os.environ.get("SCORE_THRESHOLD", 0.2))
